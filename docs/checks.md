...
| DOC-02 | README has content | 2 | Full when the root README is larger than 100 bytes and has at least 3 headings containing a recognised section keyword (install, usage, example, getting started, contribut, license, config, api, feature, setup, quickstart). Partial (1) when the README exists but is 100 bytes or smaller, or has fewer than 3 such headings. None when there is no README. |
| DOC-03 | LICENSE exists | 3 | Full if `LICENSE`, `LICENSE.md`, `LICENCE`, `COPYING` or `LICENSE.txt` exists at the root; otherwise None. |
| DOC-04 | CONTRIBUTING exists | 2 | Full if `CONTRIBUTING.md`, `CONTRIBUTING` or `.github/CONTRIBUTING.md` exists; otherwise None. |
| DOC-05 | CODE_OF_CONDUCT exists | 1 | Full if `CODE_OF_CONDUCT.md` or `.github/CODE_OF_CONDUCT.md` exists; otherwise None. |
| DOC-06 | SECURITY.md exists | 2 | Full if `SECURITY.md` or `.github/SECURITY.md` exists; otherwise None. |
...
| TST-03 | Test framework configured | 4 | Full if a framework configuration file exists anywhere in the tree (`jest.config.js/.ts/.mjs/.cjs`, `vitest.config.ts/.js/.mts`, `pytest.ini`, `.mocharc.yml/.yaml/.js`, `phpunit.xml`, `phpunit.xml.dist`), or a `pyproject.toml` mentions `pytest`, or any `*_test.go` file exists (Go's built-in test runner); otherwise None. |
| TST-04 | Coverage config exists | 3 | Full if `.nycrc`, `.nycrc.json`, `.coveragerc`, `coverage.xml`, `codecov.yml` or `.coveralls.yml` exists anywhere in the tree, or a `package.json` contains `"coverage"` (for example a coverage script); otherwise None. |
...
| DEP-05 | Dependency count | 1 | Counts declared dependencies in a root `package.json` (`dependencies` and `devDependencies` entries) or, if there is none, a root `go.mod` (direct `require` entries; `// indirect` excluded). Fewer than 50 → Full; 50–100 → Partial (0); more than 100 → None. Skipped when neither manifest is present. |
...
| SEC-05 | Branch protection indicators | 2 | Full if `CODEOWNERS`, `.github/CODEOWNERS` or `.github/branch-protection.yml` exists; otherwise None. |
...
| ACT-05 | Bus factor | 2 | Number of authors with more than 10% of non-merge commits. 3 or more → Full; 2 → Partial (1); 1 → None. Skipped when git is unavailable or there are no commits. |

Activity checks require a `.git` directory. Without one every Activity check is Skipped and the category is excluded from the score. Git commands run with a 5-second timeout.

## TODO / Technical Debt (7 points)

| ID | Check | Points | Detection |
|----|-------|-------:|-----------|
| TODO-01 | TODO/FIXME count | 3 | Counts `TODO`, `FIXME`, `HACK` and `XXX` as whole words appearing after a comment opener valid for the file's language, ignoring openers inside single-line string literals (see [TODO scanning](#todo-scanning)). Multi-line string contents are not tracked. 0 → Full; 1–20 → Partial (1); more than 20 → None. |
...
| TODO-03 | No critical TODO markers | 2 | None if any comment counted by TODO-01 also contains `SECURITY`, `VULNERABILITY` or `UNSAFE` as a whole word; otherwise Full. |
...
Directories named `node_modules`, `vendor`, `.venv`, `dist`, `build`, `target`, `out`, `.next`, `.nuxt`, `.output`, `.svelte-kit`, `__pycache__`, `.tox`, `.mypy_cache`, `.pytest_cache`, `.cache`, `.turbo`, `coverage`, `.nyc_output`, `.idea`, `.vscode` and `.git` are never scanned, and dotfiles are only read at the repository root. Scanning stops after 100,000 files.
...
Test files are excluded from the secret scan (SEC-02) and from source-file counts (STAT-01, STAT-03, TST-05).
